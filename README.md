...
- **트렌드 분석**: 지출 증감 추세 및 비교 분석
- **예산 예측**: 남은 기간 예상 지출 및 신뢰 구간
- **스마트 인사이트**: AI 기반 지출 패턴 분석 및 추천
- **커스텀 대시보드**: 위젯 순서 및 표시 설정 커스터마이징
- 종합 통계 대시보드

### Export & Reporting (내보내기 및 보고서)
- **PDF 보고서 생성**: 여행 전체 분석 보고서를 PDF로 자동 생성
  - 표지 페이지 (여행 정보, 기간, 총 지출)
  - 여행 요약 (예산 현황, 지출 통계)
  - 상세 지출 목록 (날짜별 그룹화)
  - 통계 차트 (카테고리 파이, 일별 바, 예산 번다운)
  - 분석 인사이트 (트렌드, 추천사항)
- **네이티브 공유**: 생성된 PDF를 카카오톡, 이메일 등으로 즉시 공유
- **한글 폰트 지원**: NotoSansKR 폰트로 한글 완벽 렌더링
- **통계 화면 및 여행 상세 화면에서 원클릭 접근**
...
| **다국어** | intl + ARB (148 문자열) |
| **HTTP 통신** | Dio 5.7.0 |
| **PDF 생성** | pdf 3.11.1 + printing 5.13.3 |
| **공유 기능** | share_plus 10.1.2 |
| **디자인** | Material Design 3 |
| **폰트** | Google Fonts (Lexend, NotoSansKR) |
...
| **Budget** | 예산 설정, 실시간 모니터링, 상태 추적, 예산 예측 |
| **Statistics** | 지출 통계, 차트 시각화, 트렌드 분석, 인사이트, 커스텀 대시보드 |
| **Export** | PDF 보고서 생성, 네이티브 공유 기능 |
| **Settings** | 언어 설정, 앱 설정 |
...
- **전체 테스트**: 713개 (모든 테스트 통과)
- **Unit Tests**: 도메인, 데이터, 프로바이더 (전 계층 커버)
- **Widget Tests**: 커스텀 위젯 및 UI 로직
- **Integration Tests**: 엔드투엔드 플로우 검증
- **테스트 커버리지**: 주요 기능 100% 커버
- **분석 결과**: `flutter analyze` 0개 경고
